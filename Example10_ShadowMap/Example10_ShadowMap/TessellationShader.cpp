...
TessellationShader::TessellationShader(ID3D11Device* device, HWND hwnd, bool doGrass) : BaseShader(device, hwnd)
{
	if (doGrass)
	{
		// Init geometry shaders
		initShader(L"grass_vs.cso", L"grass_hs.cso", L"grass_ds.cso", L"grass_gs.cso", L"grass_ps.cso");
	}
	else
	{
		// Init tess shaders
		initShader(L"terrainnormdisp_vs.cso", L"terrainnormdisp_hs.cso", L"terrainnormdisp_ds.cso", L"terrainnormdisp_ps.cso");
	}
...
TessellationShader::~TessellationShader()
{
	...
	// Release the grass buffer
	if(m_grassSizeBuffer)
	{
		m_grassSizeBuffer->Release();
		m_grassSizeBuffer = 0;
	}
...
void TessellationShader::initShader(WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	D3D11_BUFFER_DESC grassSizeBufferDesc;
	...
	// Setup tessellation data buffer
	...
	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	renderer->CreateBuffer(&grassBufferDesc, NULL, &m_grassBuffer);

	// Setup grass data buffer
	grassSizeBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	grassSizeBufferDesc.ByteWidth = sizeof(GrassSizeBufferTyoe);
	grassSizeBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	grassSizeBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	grassSizeBufferDesc.MiscFlags = 0;
	grassSizeBufferDesc.StructureByteStride = 0;

	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	renderer->CreateBuffer(&grassSizeBufferDesc, NULL, &m_grassSizeBuffer);
...
void TessellationShader::loadDispMapVertexShader(WCHAR* filename)
{
										     ...
										     ID3D11ShaderResourceView* rockNormMap, ID3D11ShaderResourceView* dispMap, XMFLOAT3 cameraPosition, MyLight* light[], bool showNormals)
{
	...
	// Send matrix data to VS.
	result = deviceContext->Map(m_matrixBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	dataPtr = (MatrixBufferType*)mappedResource.pData;
	dataPtr->world = worldMatrix;// worldMatrix;
	dataPtr->view = tview;
	dataPtr->projection = tproj;
	deviceContext->Unmap(m_matrixBuffer, 0);
	deviceContext->VSSetConstantBuffers(0, 1, &m_matrixBuffer);

	// Send matrix data to DS.
	result = deviceContext->Map(m_matrixBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	dataPtr = (MatrixBufferType*)mappedResource.pData;
	...
	deviceContext->PSSetShaderResources(2, 1, &grassNormMap);
	deviceContext->PSSetSamplers(0, 1, &m_sampleState);
}

void TessellationShader::setGrassShaderParameters(ID3D11DeviceContext * deviceContext, const XMMATRIX & worldMatrix, const XMMATRIX & viewMatrix, const XMMATRIX & projectionMatrix, ID3D11ShaderResourceView * texture_one, XMFLOAT3 cameraPosition, MyLight* light[], XMFLOAT4 grassClipPlane, bool showNormals)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	LightBufferType* lightPtr;
	GrassBufferType* grassPtr;
	GrassSizeBufferTyoe* sizePtr;
	MatrixBufferType* dataPtr;
	...
	// Send tessellation data to VS.
	result = deviceContext->Map(m_grassBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	grassPtr = (GrassBufferType*)mappedResource.pData;
	grassPtr->grassClipPlane = grassClipPlane;
	grassPtr->grassTessFactor = m_grassTessFactor;
	grassPtr->cameraPosition = cameraPosition;
	deviceContext->Unmap(m_grassBuffer, 0);
	deviceContext->VSSetConstantBuffers(1, 1, &m_grassBuffer);

	// Send tessellation data to VS.
	result = deviceContext->Map(m_grassSizeBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	sizePtr = (GrassSizeBufferTyoe*)mappedResource.pData;
	sizePtr->grassHeight = m_grassHeight;
	sizePtr->grassWidth = m_grassWidth;
	sizePtr->padding = XMFLOAT2(0.f, 0.f);
	deviceContext->Unmap(m_grassSizeBuffer, 0);
	deviceContext->GSSetConstantBuffers(1, 1, &m_grassSizeBuffer);
...
void TessellationShader::setMaxTessDist(int x)
{
	m_maxTessDist = x;
}

void TessellationShader::setMinTessDist(int x)
{
	m_minTessDist = x;
}

void TessellationShader::setMaxTessFactor(int x)
{
	m_maxTessFactor = x;
}

void TessellationShader::setMinTessFactor(int x)
{
	m_minTessFactor = x;
}

void TessellationShader::setGrassDensity(int x)
{
	m_grassTessFactor = x;
}

void TessellationShader::setGrassSize(float x, float y)
{
	m_grassHeight = y;
	m_grassWidth = x;
}
