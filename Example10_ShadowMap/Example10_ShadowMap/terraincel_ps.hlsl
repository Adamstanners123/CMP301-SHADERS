...
float4 doLighting(float3 normal, float3 worldPosition, float4 textureColour)
{
	...
	for (int i = 0; i < MAX_LIGHTS; i++)
	{
		...
		case POINT_LIGHT:
		{
			...
			textureColour *= doCelShading(lightVector, normal);
			break;
		}
		case SPOT_LIGHT:
		{
...
float4 slopeTexture(float2 tex, float3 normal, float3 worldPosition)
{
	...
	if (slope < 0.05)
	{
		...
		diffuseColour = doLighting(normal, worldPosition, textureColour);
	}
...
float4 main(InputType input) : SV_TARGET
{
	...
	colour = slopeTexture(input.tex, input.normal, input.worldPosition);
